    ...
    if (result instanceof app.user) {
        ...
        if (Credentials.sign === 'up') {
            ...
            organization_form.addEventListener('submit', (e) => {
                ...
                for (let i = 0; i < businesses_options.length; i++)
                    if (businesses_options[i].checked) {
                        selected_business = businesses_options[i].value;
                    }
                //
...
//the welcome
function on_finish(user, business) {
    //
    //Close the dialog
...
async function checked_business(e) {
    ...
    if (checked.length === 0) {
        ...
        for (let i = 0; i < checked.length; i++) {
            ...
            await server.exec("database", ["mutall_users", false], "query", [`INSERT INTO member(business, user) VALUES(${checked[i].id}, ${recent_user[0].user})`]);
        }
...
function add_business_db() {
    ...
    if (business_input.value.length === 0) {
        ...
        let add_business_query = `INSERT INTO business(id, name)
