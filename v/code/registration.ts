    ...
    if (result instanceof app.user){
        ...
        if(Credentials.sign==='up'){
            ...
            organization_form.addEventListener('submit', (e)=>{
                ...
                e.preventDefault();
                //
                //Variable for holding the name of the selected business
                var selected_business ='';
                //
                //GO through the options list and assign the checked business
                //as the selected one
                for(let i=0;i<businesses_options.length;i++)if(businesses_options[i].checked){selected_business=businesses_options[i].value;}
                //
                //Call a method that closes the organization dialog and welcomes the user
                on_finish(result, selected_business);
...
//
//Close the business selection dialog and redirect user to
//the welcome
function on_finish(user:app.user, business:string){
    //
    //Close the dialog
    organization_dialog.close();
    //
    //Redirect to the page that welcomes the user to the business
    // location.href="http://localhost/registration_1/v/code/welcome.html";
    // welcome(user, business);
    //
    //Display name and the business selected
    console.log(`Welcome to ${business}, ${user.name}`);
...
async function on_success(user:app.user){
    ...
    const user_business_query = `select 
                                    business.id,
                                    business.name
                                from  
                                    member 
    ...
    //
    //Depopulate the options section first
    for(let i=0;i<option_sect.childNodes.length;i++){
        option_sect.replaceChildren();
    }
    //Populate the options section with 
    if(!option_sect.hasChildNodes()){
        businesses.forEach(business=>{
            let input = document.createElement('input');
            let tag = document.createElement('label');
            let business_id:string = business.id;
...
async function multiple_selection() {
    ...
    //
    //uncheck any option that is checked
    ...
    //
    //populate the multiple selection section with businesses that
    //are in the database
...
async function checked_business(e:Event){
    ...
    if(checked.length===0){
        ...
        for(let i =0;i<checked.length;i++){
            //
            //Add the business associated with the user to the member table
            await server.exec(
                "database",
                ["mutall_users", false],
                "query",
                [`INSERT INTO member(business, user) VALUES(${checked[i].id}, ${recent_user[0].user})`]
            );
        }
...
//
//WHen we want to add a business, the following steps are taken..
function add_business_db(){
    ...
    if(business_input.value.length===0){
        ...
        let add_business_query:string = `INSERT INTO business(id, name)
                                          VALUES ('${business_value.toLowerCase().replace(" ", "_")}', '${business_value}')`;
        server.exec(
            "database",
            ["mutall_users", false],
            "query",
